                              ...
                              int *data_present, int *finished)
{
    ...
    if (error != AVERROR_EOF && input_packet.stream_index != audio_idx) goto cleanup;
                          ...
                          SwrContext **resample_context)
{
        ...
        */
        ...
        {
            fprintf(stderr, "nb of converted samples > nb of samples to be converted\n");
            exit(1);
        }
                              ...
                              int *data_present, float input_timebase)
{
    ...
    if (frame) {
        ...
        /* Update time here to know when we should cut audio. */
        current_time = input_timebase * pts;
    }
                                 ...
                                 AVCodecContext *output_codec_context, float input_timebase)
{
    ...
    if (encode_audio_frame(output_frame, output_format_context, output_codec_context, &data_written, input_timebase)) {
        av_frame_free(&output_frame);
        return AVERROR_EXIT;
    }
    av_frame_free(&output_frame);
